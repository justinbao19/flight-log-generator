...
const DURATION_HOURS_LIST = Array.from({ length: 25 }, (_, i) => i.toString());
...
function parseDuration(value: string): { hours: string; minutes: string } {
  ...
  if (colonMatch) {
    return {
      hours: String(Math.min(Number(colonMatch[1]), 24)),
      minutes: String(Math.min(Number(colonMatch[2]), 59)).padStart(2, "0"),
    };
  }
  ...
  return {
    hours: hourMatch ? String(Math.min(Number(hourMatch[1]), 24)) : "0",
    minutes: minuteMatch ? String(Math.min(Number(minuteMatch[1]), 59)).padStart(2, "0") : "00",
  };
}
...
export function DurationPicker({ label, value, onChange, readOnly, className, icon }: DurationPickerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const parsedInitial = parseDuration(value);
  const [hours, setHours] = useState(parsedInitial.hours);
  const [minutes, setMinutes] = useState(parsedInitial.minutes);
  const scrollFrameRef = useRef<ReturnType<typeof requestAnimationFrame> | null>(null);
  const hoursScrollRef = useRef<HTMLDivElement>(null);
  const minutesScrollRef = useRef<HTMLDivElement>(null);
  ...
  const setDuration = (nextHours: string, nextMinutes: string) => {
    setHours(nextHours);
  ...
  const handleHourSelect = (nextHour: string, behavior: ScrollBehavior = "smooth") => {
    const nextMinute = minutes || "00";
    scrollColumnToValue(hoursScrollRef, DURATION_HOURS_LIST, nextHour, behavior);
    setDuration(nextHour, nextMinute);
  };
  ...
  const handleColumnScroll = (
    ...
    scrollFrameRef.current = requestAnimationFrame(() => {
      ...
      const nextValue = list[index];
      if (nextValue && nextValue !== currentValue) updateValue(nextValue);
    });
  };
  ...
  useEffect(() => {
    ...
    const m = minutes || "00";
    const frame = requestAnimationFrame(() => {
      scrollColumnToValue(hoursScrollRef, DURATION_HOURS_LIST, h);
      scrollColumnToValue(minutesScrollRef, MINUTES_LIST, m);
    });
            ...
            >
              ...
              <div className="relative flex px-2 py-2">
                ...
                <div
                  ref={hoursScrollRef}
                  onScroll={(event) =>
                    handleColumnScroll(event, DURATION_HOURS_LIST, activeHour, (nextHour) =>
                      setDuration(nextHour, minutes || "00")
                    )
                  }
                  className="relative z-10 flex-1 snap-y snap-mandatory overflow-y-auto overscroll-contain px-1 scrollbar-hide custom-scrollbar"
                  style={{
                ...
                <div
                  ...
                  onScroll={(event) =>
                    handleColumnScroll(event, MINUTES_LIST, activeMinute, (nextMinute) =>
                      setDuration(hours || "0", nextMinute)
                    )
                  }
                  className="relative z-10 flex-1 snap-y snap-mandatory overflow-y-auto overscroll-contain px-1 scrollbar-hide custom-scrollbar"
                  style={{
